...
/*
 * 本文件将目光分类wk模型部署到板端，通过NNIE硬件加速进行推理。该文件提供了目光分类场景的API接口，
...
#include "osd_img.h"
#include "cnn_eyelight_classify.h"
...
//edit by rh
#define MODEL_FILE_EYELIGHT    "/userdata/models/cnn_eyelight_classify/resnet_inst.wk" // Open source model conversion
...
#define SERVER_IP "192.168.43.134"
...
HI_S32 CnnEyeLightClassifyLoadModel(uintptr_t* model, OsdSet* osds)
{
    ...
    //edit by rh
    ret = CnnCreate(&self, MODEL_FILE_EYELIGHT);
 ...
 */
HI_S32 CnnEyeLightClassifyUnloadModel(uintptr_t model)
{
    CnnDestroy((SAMPLE_SVP_NNIE_CFG_S*)model);
    SAMPLE_PRT("unload eye light classify model success\n");
...
static HI_S32 CnnEyeLightClassifyFlag(const RecogNumInfo items[], HI_S32 itemNum, HI_CHAR* buf, HI_S32 size)
{
    ...
    //offset += snprintf_s(buf + offset, size - offset, size - offset - 1, "eye light position classify: {");
    for (HI_U32 i = 0; i < itemNum; i++) {
        ...
        //    "%s%s %u:%u%%", (i == 0 ? " " : ", "), eyeLightPosition, (int)item->num, (int)score);
    ...
    //add by rh
    snprintf_s(buf, size, size - 1, "%s", eyeLightPosition);
...
HI_S32 CnnEyeLightClassifyCal(uintptr_t model, VIDEO_FRAME_INFO_S *srcFrm, VIDEO_FRAME_INFO_S *resFrm)
{
    ...
    ret = CnnEyeLightClassifyFlag(resBuf, resLen, osdBuf, sizeof(osdBuf));
     ...
     */
    //if (strcmp(osdBuf, prevOsd) != 0) {  //edit by rh
